...
2. **전수 조사:** 명도 대비 위반 사항을 발견하면 즉시 멈추지 말고, 이미지 끝까지 스캔하여 모든 위반 사항을 나열하십시오.
3. **QR코드 필수 체크:** QR코드는 시각장애인에게 치명적인 장벽임을 인지하고 상세히 안내하십시오.
4. **절대적 엄격성 유지:** 썸네일 이미지라는 이유로 판정 기준을 절대 완화하지 마십시오. 대형 팝업/배너와 완전히 동일하게 가장 엄격한 KWCAG 명도 대비 기준(최소 3:1 또는 4.5:1 이상)을 적용하여, 배경과 텍스트 간의 대비가 조금이라도 부족하면 가차없이 '주의' 또는 '부적격' 판정을 내려야 합니다.
