...
# 5. 시스템 프롬프트 모듈화 (DRY 원칙 적용)

# [공통 원칙] 모든 카테고리에 공통으로 적용되는 강력한 검사 기준
COMMON_RULES = """
[SYSTEM SETTING: RIGOROUS ANALYTICAL MODE]
당신은 'KWCAG 정보접근성 품질 관리관'입니다. 

**[작업 원칙: 정밀 분석 (텍스트가 있는 경우)]**
...
4. **절대적 엄격성 유지:** 이미지의 종류(썸네일, 팝업 등)와 상관없이 판정 기준을 절대 완화하지 마십시오. 가장 엄격한 KWCAG 명도 대비 기준(텍스트 크기에 따라 최소 3:1 또는 4.5:1 이상)을 적용하여, 배경과 텍스트 간의 대비가 조금이라도 부족하면 가차없이 '주의' 또는 '부적격' 판정을 내려야 합니다.
5. **로고 제외 필수:** 상단 또는 하단에 위치한 기관 로고(예: 경기도평생교육진흥원 등)는 명도 대비 검사 및 텍스트 추출(Full OCR) 대상에서 **반드시 제외**하십시오.
"""

# [공통 출력 포맷] 텍스트가 있는 이미지에 대한 표준 출력 1~3단계
COMMON_OUTPUT_FORMAT_TEXT_EXISTS = """
**1단계: 명도 대비 판정**
- ✅ 적격 / ⚠️ 주의 / ❌ 부적격
...
**3단계: 텍스트 소스 (전체 추출)** {text_source_note}
...
# [공통 분류 원칙] 텍스트 유무 판단 로직
COMMON_CLASSIFICATION_RULE = """
**[작업 원칙: 이미지 성격 분류 (텍스트 유무 판단)]**
가장 먼저 이 이미지가 다음 중 어느 것인지 면밀하게 판단하십시오.
- **[텍스트를 포함하는 이미지]**: 의도적으로 정보 전달을 위해 텍스트를 넣어 제작한 이미지 (예: 썸네일, 카드뉴스, 인포그래픽, 포스터 이미지 등)
...
목표: 이미지 내 텍스트를 끝까지 완전하게 추출하고, 명도 대비 위반 및 QR코드 접근성을 전수 조사하는 것입니다.
...
"""
        ...
        return COMMON_RULES + COMMON_CLASSIFICATION_RULE + f"""
...
"""
        ...
        return COMMON_RULES + COMMON_CLASSIFICATION_RULE + f"""
목표: 썸네일 이미지 내 텍스트 유무를 가장 먼저 판단하고, 텍스트가 있는 경우 명도 대비 위반 및 텍스트 추출을 수행합니다.

**[출력 포맷: 텍스트를 포함하지 않는 이미지]**
만약 텍스트를 포함하지 않는 이미지라면 **반드시 아래 내용만 정확히 출력**하고 종료하십시오:
    ...
    if image is not None:
        ...
        if st.button("🔍 점검 시작", type="primary", key=f"btn_{category}"):
            with st.spinner('최신 AI가 이미지를 정밀 분석 중입니다...'):
                try:
                    # 카테고리별 프롬프트 분기 처리
                    prompt_to_use = get_system_prompt(category)
