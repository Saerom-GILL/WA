...
4. **절대적 엄격성 유지:** 대형 팝업/배너라 하더라도 판정 기준을 절대 완화하지 마십시오. 가장 엄격한 KWCAG 명도 대비 기준(텍스트 크기에 따라 최소 3:1 또는 4.5:1 이상)을 적용하여, 배경과 텍스트 간의 대비가 조금이라도 부족하면 가차없이 '주의' 또는 '부적격' 판정을 내려야 합니다.
...
가장 먼저 이 이미지가 다음 중 어느 것인지 면밀하게 판단하십시오.
- **[텍스트를 포함하는 이미지]**: 의도적으로 정보 전달을 위해 텍스트를 넣어 제작한 썸네일 이미지
- **[텍스트를 포함하지 않는 이미지]**: 일반적인 행사 사진, 풍경 사진, 인물 사진 등. (주의: 사진 배경에 우연히 찍힌 현수막이나 간판의 텍스트가 있다고 해서 '텍스트를 포함하는 이미지'로 분류해서는 안 됩니다.)
...
2. **전수 조사:** 명도 대비 위반 사항을 발견하면 즉시 멈추지 말고, 이미지 끝까지 스캔하여 모든 위반 사항을 나열하십시오.
3. **QR코드 필수 체크:** QR코드는 시각장애인에게 치명적인 장벽임을 인지하고 상세히 안내하십시오.
4. **절대적 엄격성 유지:** 썸네일 이미지라 하더라도 판정 기준을 절대 완화하지 마십시오. 가장 엄격한 KWCAG 명도 대비 기준(텍스트 크기에 따라 최소 3:1 또는 4.5:1 이상)을 적용하여, 배경과 텍스트 간의 대비가 조금이라도 부족하면 가차없이 '주의' 또는 '부적격' 판정을 내려야 합니다.
    ...
    if image is not None:
        ...
        if st.button("🔍 점검 시작", type="primary", key=f"btn_{category}"):
            with st.spinner('최신 AI가 이미지를 정밀 분석 중입니다...'):
                try:
                    # 카테고리별 프롬프트 분기 처리
                    if category == '게시글 내 삽입 이미지':
                        prompt_to_use = SYSTEM_PROMPT_POST
                    elif category == '썸네일':
                        prompt_to_use = SYSTEM_PROMPT_THUMBNAIL
                    else:
